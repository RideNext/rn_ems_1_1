...
const mapDispatch = (dispatcher: IDispatcher) => ({
  refreshprofile: () => dispatcher.dispatch(avaliablesystemServersReloadAction)
});
...
const settings: { [key: string]: DialogSettings } = {
  ...
  [RefreshprofileDialogMode.RefreshprofileTable]: {
    dialogTitle: "Do you want to refresh the System Performance table?",
    dialogDescription: "",
    applyButtonText: "Yes",
...
type RefreshprofileDialogComponentState = systemdata & { isNameValid: boolean, isHostSet: boolean };
      ...
      <Dialog open={this.props.mode !== RefreshprofileDialogMode.None}>
        ...
        <DialogActions>
          <Button aria-label="dialog-confirm-button" onClick={(event) => {
             axios.get(baseuri + "/proxyapi/run-script").then(()=>{
              this.onRefresh();
             })
